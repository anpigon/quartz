...
> [!info] 정보
> 웹사이트에 접속했을 때 **_"404 Either this page is private or doesn't exist."_** 라는 메시지가 표시된다면, 이는 `content` 폴더 내부에 `index.md` 파일이 없는 것이 원인일 수 있습니다. 이 문제를 해결하기 위해서는 `content` 폴더 안에 `index.md` 파일을 생성해 주세요.
